(($, { ajax, behaviors }) => {
  behaviors.commercePackagingShipmentPackager = {
    attach(context) {
      $(context).find('.shipment-packager__area').sortable({
        items: '> .draggable',
        connectWith: '.shipment-packager__area',
         ...
         */
        update(event, ui) {
          const itemRegion = ui.item.closest('.shipment-packager__area');
